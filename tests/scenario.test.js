...
const createRuntime = (options = {}) => {
  ...
  const run = (ctx, next = noop) => {
    return sessions(ctx, () => scenarios.middleware()(ctx, next));
  };
  return { scenarios, run, store };
};

test('scenario uses named steps stored in session', async () => {
  const events = [];
  const registration = defineScenario()({
    id: 'registration',
    ...
    steps: {
      ...
      'read-name': ({ ctx }) => {
        // Бизнес-операция выполняется до complete, поэтому при ошибке шаг останется активным.
  ...
  await run(first, () => first.scenario.start(registration));
  assert.equal(first.prompt, 'name');
  assert.equal(store.get('user').scenario.step, 'read-name');
...
test('stay persists a data patch without changing the named step', async () => {
  ...
  const { scenarios, run, store } = createRuntime();
  scenarios.register(flow);

  const ctx = { key: 'user' };
  await run(ctx, () => ctx.scenario.start(flow));
  assert.deepEqual(store.get('user').scenario, {
    id: 'attempts', step: 'validate', data: { attempts: 1 }, expiresAt: undefined,
  });
});

test('failed final step leaves scenario available for retry', async () => {
  let saves = 0;
  const flow = defineScenario()({
    id: 'reliable',
    initialStep: 'prepare',
  ...
  const first = { key: 'user' };
  await run(first, () => first.scenario.start(flow));
  await assert.rejects(() => run({ key: 'user', fail: true }), /database unavailable/);
  assert.equal(store.get('user').scenario.step, 'save');
...
test('another definition with the same id is rejected before state is written', async () => {
  const original = defineScenario()({
    id: 'duplicate', initialStep: 'one', createData: () => ({}),
    steps: { one: () => transition.stay() },
  });
  const replacement = defineScenario()({
    id: 'duplicate', initialStep: 'two', createData: () => ({}),
    steps: { two: () => transition.stay() },
  });
  const { scenarios, run, store } = createRuntime();
  scenarios.register(original);
  const ctx = { key: 'user' };

  await assert.rejects(
    () => run(ctx, () => ctx.scenario.start(replacement)),
    /another definition/,
  );
  assert.equal(store.get('user').scenario, undefined);
});

test('expired scenario is removed and does not consume the update', async () => {
  let now = 100;
  ...
  const { scenarios, run, store } = createRuntime({ now: () => now });
  scenarios.register(flow);
  const first = { key: 'user' };
  await run(first, () => first.scenario.start(flow));
...
test('split middleware lets global commands run before scenario interception', async () => {
  let stepCalls = 0;
  const flow = defineScenario()({
  ...
  const run = (ctx, globalMiddleware) => sessions(ctx, () => {
    return scenarios.controllerMiddleware()(ctx, () => {
      return globalMiddleware(ctx, () => scenarios.interceptMiddleware()(ctx, noop));
    });
  });

  const first = { key: 'user' };
  await run(first, (ctx) => ctx.scenario.start(flow));
  assert.equal(store.get('user').scenario.step, 'wait');

  const cancel = { key: 'user', command: 'cancel' };
  await run(cancel, async (ctx, next) => {
    assert.equal(ctx.scenario.current, 'global-command');
    if (ctx.command !== 'cancel') return next();
    assert.equal(ctx.scenario.cancel(), true);
    return undefined;
  });

  assert.equal(stepCalls, 0);
  assert.equal(store.get('user').scenario, undefined);
  assert.equal(cancel.scenario.active, false);
  assert.equal(cancel.scenario.current, undefined);
  assert.equal(cancel.scenario.cancel(), false);
});

test('session serialization protects concurrent scenario updates locally', async () => {
  ...
  assert.equal(store.get('user').scenario.data.count, 2);
});
...
test('definition validation rejects unknown initial steps', () => {
  ...
  assert.throws(() => new ScenarioEngine().register(invalid), /no initial step/);
});
