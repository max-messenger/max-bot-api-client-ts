...
> ⚠️ MAX требует, чтобы webhook был доступен по HTTPS. 
> Для локальной разработки используйте туннель (например, `VK Tunnel`) или любой другой прокси, 
> который пробрасывает HTTPS на ваш локальный порт.